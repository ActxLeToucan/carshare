<template>
    <router-link
        ...
        class="flex items-center justify-center w-full h-fit py-2 px-4 text-slate-500 dark:text-slate-400 text-xl font-bold bg-white
               dark:bg-slate-700 rounded-md border-2 border-slate-200 dark:border-slate-600 outline-none focus:border-teal-500 focus:dark:border-teal-500 transition-all"
    >
    ...
    watch: {
        '$route.hash': function (newVal, oldVal) {
            if (this.href == newVal || (this.default && !newVal)) {
                ...
                this.$refs["btn"].$el.classList.add('bg-teal-500', 'dark:bg-teal-500', 'text-white', 'dark:text-slate-700', 'border-teal-600');
            } else {
                this.$refs["btn"].$el.classList.remove('bg-teal-500', 'dark:bg-teal-500', 'text-white', 'dark:text-slate-700', 'border-teal-600');
