...
<div class="md:show-up flex flex-col grow" >
        ...
        <div class="flex flex-col grow justify-evenly items-center p-4 space-y-4">
                        <p class="text-2xl text-teal-500 py-2 font-bold mx-auto"> {{ lang.FUTURE_TRIPS }} </p>
...
function search(obj) {
    ...
    API.execute_logged(API.ROUTE.TRAVELS.MY.PASSENGER + obj.pagination + "&query="+value, API.METHOD.GET, User.CurrentUser?.getCredentials()).then(data => {
...
export default {
    ...
    methods: {
        ...
        deleteAccount(popup) {
            ...
            API.execute_logged(API.ROUTE.TRAVELS.MY.PASSENGER + "/" + this.selectedUser.id + this.pagination, API.METHOD.DELETE, User.CurrentUser?.getCredentials()).then((data) => {
        ...
        updateAccount() {
            ...
            API.execute_logged(API.ROUTE.TRAVELS.MY.PASSENGER + "/" + this.selectedUser.id, API.METHOD.PATCH, User.CurrentUser?.getCredentials(), newData).then((data) => {
                for (const prop of props) {
